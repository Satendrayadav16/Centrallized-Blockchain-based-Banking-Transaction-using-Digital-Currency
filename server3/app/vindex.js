...
const {getETCD} = require('../userWallet/getEtcd');
...
app.get('/blocks', async(req, res) => {
    let etcdData = await getETCD();
    res.json(etcdData.CHAIN);
});
...
app.post('/ABC', async (req, res) => {
    ...
    let etcdData = await getETCD();
    res.json(existingAccountByCN(citizenshipNumber, etcdData.CHAIN));
});

app.post('/ABA', async(req, res) => {
    const {address} = req.body;
    let etcdData = await getETCD();
    res.json(existingAccountByAddress(address, etcdData.CHAIN));
});
...
app.post('/bal', async (req, res) => {
    const {address} = req.body;

    let etcdData = await getETCD();

    if (existingAccountByAddress(address, etcdData.CHAIN).length !== 0) {
        let balance = balanceCalculator(etcdData.CHAIN, address);
        res.json(balance);
    } else {
...
app.post('/gt', async(req, res) => {
    ...
    if (existingAccountByAddress(address, etcdData.CHAIN).length !== 0) {
        let transactions = getMinedReceivedTransactionsByAddress(address, etcdData.CHAIN);
...
app.get('/getAllTransactions', async(req, res) => {
    ...
    let transactionList = getAllTransactions(etcdData.CHAIN);
...
app.post('/getTransactionByHash', async(req, res) => {
    ...
    let transactionList = getTransactionByHash(etcdData.CHAIN, hash);
    res.json(transactionList);
});


app.post('/getBlockByHash', async (req, res) => {
    let {hash} = req.body;
    let etcdData = await getETCD();

    let blockList = getBlockByHash(etcdData.CHAIN, hash);
    res.json(blockList);
});

app.post('/getBlockByHeight', async(req, res) => {
    let {height} = req.body;
    let etcdData = await getETCD();

    let blockList = getBlockByHeight(etcdData.CHAIN, height);
    res.json(blockList);
});

app.get('/getTotalAmountCirculating', async(req, res) => {
    ...
    res.json(getTotalAmountCirculating(etcdData.CHAIN, address));
});

app.post('/getTransactionByBlockHeight', async(req, res) => {
    let {height} = req.body;
    let etcdData = await getETCD();

    let transactionList = getTransactionByBlockHeight(etcdData.CHAIN, height);
    res.json(transactionList);
});
...
app.get('/getAllAccount', async(req, res) => {
    ...
    let accountList = getAllAccounts(etcdData.CHAIN);
